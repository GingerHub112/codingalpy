...
bg = pg.transform.scale(pg.image.load("bg.png"), ((SCREEN_WIDTH, SCREEN_HEIGHT)))
...
spr1 = Sprite(pg.Color("black"), 20, 30)
...
while running:
    ...
    if not won:
        ...
        if spr1.rect.colliderect(spr2.rect):
            all_sprs.remove(spr2)
            won = True

        screen.blit(bg, (0,0))
        all_sprs.draw(screen)
        if won:
            win_text = font.render("You win", True, pg.Color("black"))
